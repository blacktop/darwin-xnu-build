...
- `00_setup_coreentitlements.sh`: copies every CoreEntitlements V2 header from the KDK selected by `build.sh` into `EXTERNAL_HEADERS`, plus Apple's `V2/Kernel.h` from the KDK's `System/Library/KernelSupport` (borrowed from another installed 26.x KDK when the target KDK predates it). It also deletes the obsolete `os/firehose_buffer_private.h` stub that earlier revisions generated (see the notes below).
...
- `define_esr_ec_pac_fail_unconditionally.patch`: keeps `ESR_EC_PAC_FAIL` available when compiling the `xnu_libraries` target as plain arm64, matching its unconditional use in `sleh.c`.
...
The same script used to write a made-up `CoreEntitlements/V2/Kernel.h` declaring a `coreentitlements_kernel_api` struct with 16 invented members. Apple's real `CEKernelAPI_t` (`struct _CEKernelAPI`, 45 function pointers, verified against the DWARF in the KDK's `kernel.release.t6020.dSYM`) ships in every KDK from 26.4 on under `System/Library/KernelSupport/CoreEntitlements/V2/Kernel.h`, a directory already on xnu's include path, and the stub in `EXTERNAL_HEADERS` shadowed it. VMAPPLE kernels compiled anyway because every use of the table sits under `CODE_SIGNING_MONITOR`, which VMAPPLE does not enable; any SPTM SoC config (T6020 and friends, including the `xnu_libraries` build Apple's unit tests use) failed in `ubc_subr.c`. The script now copies Apple's header.
